...
- **Both sides walk away with proof.** A YES gets the buyer their own copy of the same PDF receipt the merchant gets, not just a "thanks" text, since they're a party to the sale, not just an approver of it. A NO gets a plain acknowledgement instead, there's nothing to hand over since nothing was recorded.
...
- `auth_info_baileys/` (the WhatsApp session) gets wiped on restart, meaning a re-link. Set `WHATSAPP_PHONE_NUMBER` (digits, country code, no `+`) to link via a short pairing code instead of a QR — a hosted log viewer timestamps every printed line separately, which breaks an ASCII QR's grid the moment it's copied or reflowed, a pairing code has no such problem.
...
**A second, separate consequence of being two independent services, not just one hidden behind ephemeral storage:** `agent/whatsapp.js` and `server.js` each get their own filesystem, so `data/` on one is invisible to the other even *while both are running*, not just across a restart. The agent writes business names and per-receipt details (item, buyer name, PDF metadata) to its own local `data/`; the dashboard's `/api/merchants` and `/api/receipts` read from a `data/` that only the dashboard's own container ever writes to, which is to say, never. Concretely: a merchant's business name will show as a raw Credora ID instead of "Amina's Rice and Grains," and a merchant's receipt list in the dashboard drawer will stay empty, even right after a real, successful, on-chain sale. Tier, credit score, streak, volume, and `confirmedCount` are unaffected by any of this, they're read directly from chain events client-side in `dashboard.html`, no local store involved, so the numbers a lender actually needs to trust are never at risk here. Fixing this properly means giving the dashboard a way to read the agent's local store across the service boundary (an internal API on the agent, or a shared external store), deliberately out of scope for the free-tier setup described here.

`agent/whatsapp.js` normally has no HTTP surface at all, since it's just a WebSocket connection to WhatsApp, so `agent/healthServer.js` adds a minimal `/health` endpoint purely so Render treats it as a Web Service instead of rejecting it (Render's free tier has no Background Worker option, paid plans start at $7/month for that). That same endpoint is also what keeps a free instance awake: Render sleeps free Web Services after 15 minutes with no HTTP traffic. `.github/workflows/keep-alive.yml` pings both services' health endpoints every 5 minutes via GitHub Actions (free and unmetered, since this repo is public) so neither one ever idles out, no external cron-job.org/UptimeRobot account needed. It does not, and cannot, stop the disk-wiping restarts described above, those are a separate problem this doesn't touch.

If this needs to survive restarts without re-linking WhatsApp, the two real fixes are a small paid persistent disk on Render (roughly $7 to 10/month all in), or a genuinely free always-on VM with real persistent disk, like Oracle Cloud's Always Free tier, at the cost of setting up and maintaining the VM yourself instead of a git-push deploy.
...
- Give `server.js` a way to read `agent/whatsapp.js`'s local store across the Render service boundary (internal API, or a shared external store), so business names and the per-receipt list show up on a split-service deploy instead of only working when both processes share one filesystem
